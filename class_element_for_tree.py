...
class BinaryTreeElement:
    def __init__(
        self,
        dict_elements: dict[int, dict[str, str]],
        conclusion: str,
        main_premises: list[str],
        premises_option: list[str],
        conclusion_option: list[str],
        window,
    ) -> None:
    ...
    def calculate_conclusion_entropy(self) -> float:
        ...
        for number in self.return_conclusion_tab():
            if number == 0:
                ...
                conclusion_entropy += -(number / len(self.dict_elements)) * log2(
                    number / len(self.dict_elements)
                )
        return conclusion_entropy

    def calculate_entropy_for_premise_return_info(
        self, main_premise: str, premise: str
    ) -> float:
        ...
        try:
            for key, value in self.dict_elements.items():
                if (
                    value[main_premise] == premise
                    and value[self.conclusion] == self.conclusion_option[0]
                ):
                    positive_data_to_entropy[0] += 1
                elif (
                    value[main_premise] == premise
                    and value[self.conclusion] == self.conclusion_option[1]
                ):
                    positive_data_to_entropy[1] += 1
                elif (
                    value[main_premise] == premise
                    and value[self.conclusion] == self.conclusion_option[2]
                ):
                    positive_data_to_entropy[2] += 1
            # print(positive_data_to_entropy)
            for number in range(len(self.return_conclusion_tab())):
                negative_data_to_entropy[number] = (
                    self.return_conclusion_tab()[number]
                    - positive_data_to_entropy[number]
                )
            ...
            return positive_premise_entropy * (
                sum(positive_data_to_entropy) / len(self.dict_elements)
            ) + negative_premise_entropy * (
                sum(negative_data_to_entropy) / len(self.dict_elements)
            )
        except AttributeError:
            return ""
    ...
    def return_entropy_tab(self) -> list[float]:
        ...
        if len(self.main_premises) >= 1:
            for option in self.premises_option[:4]:
                value = self.calculate_entropy_for_premise_return_info(
                    main_premise=self.main_premises[0], premise=option
                )
                entropy.append(value)
        if len(self.main_premises) >= 2:
            for option in self.premises_option[4:6]:
                value = self.calculate_entropy_for_premise_return_info(
                    main_premise=self.main_premises[1], premise=option
                )
                entropy.append(value)
        if len(self.main_premises) >= 3:
            for option in self.premises_option[6:9]:
                value = self.calculate_entropy_for_premise_return_info(
                    main_premise=self.main_premises[2], premise=option
                )
                entropy.append(value)
        if len(self.main_premises) >= 4:
            for option in self.premises_option[9:12]:
                value = self.calculate_entropy_for_premise_return_info(
                    main_premise=self.main_premises[3], premise=option
                )
                entropy.append(value)
        if len(self.main_premises) >= 5:
            for option in self.premises_option[12:]:
                value = self.calculate_entropy_for_premise_return_info(
                    main_premise=self.main_premises[4], premise=option
                )
                entropy.append(value)
        return entropy
    ...
    def return_left_branch(self) -> dict[int, dict[str, str]]:
        ...
        try:
            for key in self.dict_elements.keys():
                if (
                    self.dict_elements[key][self.return_best_main_premise()]
                    == self.find_best_entropy()
                ):
    ...
    def draw_element_on_tree(self, position_x: int, position_y: int) -> None:
        try:
            position_no = len(str(self.return_best_main_premise())) + len(
                str(self.find_best_entropy())
            )
            position_no = position_x + position_no * 8
            position_yes = len(str(self.return_best_main_premise())) + len(
                str(self.find_best_entropy())
            )
            ...
            label_node = Label(
                self.window,
                text=f"{self.return_best_main_premise()} {self.find_best_entropy()}",
                justify="center",
    ...
    def __str__(self) -> str:
        if (
            self.find_best_entropy() != self.conclusion_option[0]
            and self.find_best_entropy() != self.conclusion_option[1]
            and self.find_best_entropy() != self.conclusion_option[2]
            and self.calculate_conclusion_entropy() != 0.0
        ):
            show_info_gain = ""
            print(f"I (konkluzja) -- {self.calculate_conclusion_entropy()}")
            print("j\t\t\tI-Ej")
            print(35 * "-")
            for index, unit_information_gain in enumerate(
                self.return_information_gain()
            ):
